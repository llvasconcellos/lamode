    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
        ...
        switch ($this->getConfigData('urlmethod')){
            ...
            case 0:
                ...
                if($correiosReturn !== false){
                    ...
                    //METODO ENVIO GRATUITO PARA JOINVILLE
                    $cep = (int)str_replace(".", "", str_replace("-", "", str_replace(" ", "", $this->_toZip)));
                    if((89200000 <= $cep)&&($cep<= 89239999)){
	                    $this->_apendShippingReturn((string)"000", floatval(0), 0);
	                    $existReturn = true;
                    }
                    
                    //$this->_fromZip = Mage::getStoreConfig('shipping/origin/postcode', $this->getStore());
        			//$this->_toZip = $request->getDestPostcode();
                    //var_dump($this->_toZip);
                    //die();
    ...
    protected function _throwError($message, $log = null, $line = 'NO LINE', $custom = null){
		...
		$config = Mage::getStoreConfig('carriers/pedroteixeira_correios');
		
		if((array_key_exists('default_value_pac', $config)) && (strlen($config['default_value_pac'])>0)){
			$pac = str_replace(',', '.', str_replace('.', '', $config['default_value_pac']));
		}
		else $pac = 14;
		
		if((array_key_exists('default_value_sedex', $config)) && (strlen($config['default_value_sedex'])>0)){
			$sedex = str_replace(',', '.', str_replace('.', '', $config['default_value_sedex']));
		}
		else $sedex = 34;
		
		$this->_apendShippingReturn("40010", $sedex, "2 dias");
		$this->_apendShippingReturn("41106", $pac, "6 dias");
