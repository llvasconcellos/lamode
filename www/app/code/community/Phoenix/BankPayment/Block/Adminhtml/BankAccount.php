...
class Phoenix_BankPayment_Block_Adminhtml_BankAccount extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getRowTemplateHtml($i=0)
    {
		...
		$html .= '<label>'.$this->__('Agência').'</label>';
		$html .= '<input class="input-text" type="text" name="'.$this->getElement()->getName().'[account_agencia][]" value="' . $this->_getValue('account_agencia/'.$i) . '" '.$this->_getDisabled().' />';
