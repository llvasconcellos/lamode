/**
 * Seadragon Ajax 0.8.4 (build 18035 on 2009-10-22) Copyright (c) Microsoft
 * Corporation. All rights reserved. For more information, visit:
...
Seadragon.Point = function(x, y) {
	...
	this.equals = function(point) {
		return (point instanceof Seadragon.Point) && (self.x === point.x)
				&& (self.y === point.y);
	};
...
Seadragon.Rect = function(x, y, width, height) {
	...
	this.getCenter = function() {
		return new Seadragon.Point(self.x + self.width / 2.0, self.y
				+ self.height / 2.0);
	};
	...
	this.equals = function(other) {
		return (other instanceof Seadragon.Rect) && (self.x === other.x)
...
Seadragon.Spring = function(initialValue) {
	...
	this.update = function() {
		...
		currentValue = (currentTime >= targetTime) ? targetValue : startValue
				+ (targetValue - startValue)
				* transform((currentTime - startTime)
...
Seadragon.Utils = function() {
	...
	(function() {
		...
		if (app == "Microsoft Internet Explorer" && !!window.attachEvent
				&& !!window.ActiveXObject) {
			var ieOffset = ua.indexOf("MSIE");
			browser = Browser.IE;
			browserVersion = parseFloat(ua.substring(ieOffset + 5, ua.indexOf(
					";", ieOffset)));
	...
	this.getMousePosition = function(event) {
		...
		var result = new Seadragon.Point();
		if (event.type == "DOMMouseScroll" && browser == Browser.FIREFOX
				&& browserVersion < 3) {
	...
	this.getMouseScroll = function(event) {
		...
		if (typeof (event.wheelDelta) == "number") {
			...
			Seadragon.Debug
					.fail("Unknown event mouse scroll, no known technique.");
		}
	...
	this.removeEvent = function(elmt, eventName, handler, useCapture) {
		...
		if (elmt.removeEventListener) {
			...
			Seadragon.Debug
					.fail("Unable to detach event handler, no known technique.");
		}
	};
	...
	this.makeAjaxRequest = function(url, callback) {
		...
		if (async) {
			...
			var callback = function() {
				window.setTimeout(Seadragon.Utils.createCallback(null, actual,
						req), 1);
			};
		}
		...
		try {
			...
			Seadragon.Debug.log(e.name + " while making AJAX request: "
					+ e.message);
	...
	this.parseXml = function(string) {
		...
		if (window.ActiveXObject) {
			try {
				...
				Seadragon.Debug.log(e.name + " while parsing XML (ActiveX): "
						+ e.message);
			}
			...
			try {
				...
				Seadragon.Debug.log(e.name + " while parsing XML (DOMParser): "
						+ e.message);
			}
		} else {
...
(function() {
	...
	(function() {
		if (isIE) {
			Seadragon.Utils.addEvent(document, "mousedown", onGlobalMouseDown,
					false);
			Seadragon.Utils.addEvent(document, "mouseup", onGlobalMouseUp,
					false);
		} else {
			Seadragon.Utils.addEvent(window, "mousedown", onGlobalMouseDown,
					true);
	...
	Seadragon.MouseTracker = function(elmt) {
		...
		function startTracking() {
			if (!tracking) {
				...
				Seadragon.Utils.addEvent(elmt, "mousewheel", onMouseScroll,
						false);
		...
		function stopTracking() {
			if (tracking) {
				Seadragon.Utils.removeEvent(elmt, "mouseover", onMouseOver,
						false);
				Seadragon.Utils
						.removeEvent(elmt, "mouseout", onMouseOut, false);
				Seadragon.Utils.removeEvent(elmt, "mousedown", onMouseDown,
						false);
				Seadragon.Utils.removeEvent(elmt, "mouseup", onMouseUp, false);
				Seadragon.Utils.removeEvent(elmt, "mousewheel", onMouseScroll,
						false);
		...
		function captureMouse() {
			if (!capturing) {
				if (isIE) {
					Seadragon.Utils.removeEvent(elmt, "mouseup", onMouseUp,
							false);
					Seadragon.Utils
							.addEvent(elmt, "mouseup", onMouseUpIE, true);
					Seadragon.Utils.addEvent(elmt, "mousemove", onMouseMoveIE,
							true);
				} else {
					Seadragon.Utils.addEvent(window, "mouseup",
							onMouseUpWindow, true);
					Seadragon.Utils.addEvent(window, "mousemove", onMouseMove,
							true);
				}
				capturing = true;
			}
		...
		function releaseMouse() {
			if (capturing) {
				if (isIE) {
					Seadragon.Utils.removeEvent(elmt, "mousemove",
							onMouseMoveIE, true);
					Seadragon.Utils.removeEvent(elmt, "mouseup", onMouseUpIE,
							true);
					Seadragon.Utils.addEvent(elmt, "mouseup", onMouseUp, false);
				} else {
					Seadragon.Utils.removeEvent(window, "mousemove",
							onMouseMove, true);
					Seadragon.Utils.removeEvent(window, "mouseup",
							onMouseUpWindow, true);
				}
				capturing = false;
			}
		...
		function onMouseOver(event) {
			...
			if (typeof (self.enterHandler) == "function") {
				try {
					...
					Seadragon.Debug
							.error(e.name + " while executing enter handler: "
									+ e.message, e);
				}
			}
		}
		function onMouseOut(event) {
			...
			var to = event.relatedTarget ? event.relatedTarget
					: event.toElement;
		...
		function onMouseDown(event) {
			...
			if (typeof (self.pressHandler) == "function") {
				try {
					...
					Seadragon.Debug
							.error(e.name + " while executing press handler: "
									+ e.message, e);
				}
			}
		...
		function onMouseUp(event) {
			...
			if (typeof (self.releaseHandler) == "function") {
				try {
					...
					Seadragon.Debug.error(e.name
							+ " while executing release handler: " + e.message,
							e);
				}
			}
		...
		function handleMouseClick(event) {
			...
			if (typeof (self.clickHandler) == "function") {
				try {
					...
					Seadragon.Debug
							.error(e.name + " while executing click handler: "
									+ e.message, e);
				}
			}
		}
			...
			if (typeof (self.scrollHandler) == "function") {
				if (delta) {
					try {
						...
						Seadragon.Debug.error(e.name
								+ " while executing scroll handler: "
								+ e.message, e);
					}
				}
				Seadragon.Utils.cancelEvent(event);
...
(function() {
	...
	Seadragon.ImageLoader = function() {
		...
		this.loadImage = function(src, callback) {
			...
			var func = Seadragon.Utils.createCallback(null, onComplete,
					callback);
...
(function() {
	...
	Seadragon.Button = function(tooltip, srcRest, srcGroup, srcHover, srcDown,
		...
		function releaseHandler(tracker, position, insideElmtPress,
				insideElmtRelease) {
			if (insideElmtPress && insideElmtRelease) {
...
Seadragon.TileSource = function(width, height, tileSize, tileOverlap, minLevel,
		maxLevel) {
	var self = this;
	...
	this.maxLevel = maxLevel ? maxLevel : Math.ceil(Math.log(Math.max(width,
			height))
			/ Math.log(2));
	...
	this.getTileAtPoint = function(level, point) {
		...
		if (point.x >= 0.0 && point.x <= 1.0) {
			...
			tx = Math.ceil(scaledSize.x / self.tileSize)
					* Math.floor(pixel.x / scaledSize.x)
					+ Math
							.floor(((scaledSize.x + (pixel.x % scaledSize.x)) % scaledSize.x)
									/ self.tileSize);
		}
		if (point.y >= 0.0 && point.y <= normHeight) {
			ty = Math.floor(pixel.y / self.tileSize);
		} else {
			ty = Math.ceil(scaledSize.y / self.tileSize)
					* Math.floor(pixel.y / scaledSize.y)
					+ Math
							.floor(((scaledSize.y + (pixel.y % scaledSize.y)) % scaledSize.y)
									/ self.tileSize);
		}
	...
	this.getTileBounds = function(level, x, y) {
		...
		return new Seadragon.Rect(px * scale, py * scale, sx * scale, sy
				* scale);
	};
	...
	this.tileExists = function(level, x, y) {
		...
		return level >= self.minLevel && level <= self.maxLevel && x >= 0
...
Seadragon.DziTileSource = function(width, height, tileSize, tileOverlap,
		tilesUrl, fileFormat, displayRects) {
...
(function() {
	...
	function getError(e) {
		if (!(e instanceof DziError)) {
			Seadragon.Debug.error(e.name + " while creating DZI from XML: "
					+ e.message);
	...
	function processResponse(xhr, tilesUrl) {
		if (!xhr) {
			...
			throw new DziError(Seadragon.Strings.getString("Errors.Status",
					status, statusText));
		}
		var doc = null;
	...
	function processDzi(imageNode, tilesUrl) {
		...
		if (!Seadragon.Utils.imageFormatSupported(fileFormat)) {
			throw new DziError(Seadragon.Strings.getString(
					"Errors.ImageFormat", fileFormat.toUpperCase()));
		}
		...
		for ( var i = 0; i < dispRectNodes.length; i++) {
			...
			dispRects.push(new Seadragon.DisplayRect(parseInt(rectNode
					.getAttribute("X"), 10), parseInt(rectNode
					.getAttribute("Y"), 10), parseInt(rectNode
					.getAttribute("Width"), 10), parseInt(rectNode
					.getAttribute("Height"), 10), 0, parseInt(dispRectNode
					.getAttribute("MaxLevel"), 10)));
		}
		return new Seadragon.DziTileSource(width, height, tileSize,
				tileOverlap, tilesUrl, fileFormat, dispRects);
	}
	...
	Seadragon.DziTileSource.createFromXml = function(xmlUrl, xmlString,
			callback) {
		var async = typeof (callback) == "function";
		var error = null;
		if (!xmlUrl) {
		...
		if (async) {
			if (xmlString) {
				window.setTimeout(function() {
					var source = finish(processXml, Seadragon.Utils
							.parseXml(xmlString));
		...
		if (xmlString) {
			...
			return finish(processResponse, Seadragon.Utils
...
Seadragon.Viewport = function(containerSize, contentSize) {
	...
	function getMinZoom() {
		...
		if (Seadragon.Config.minZoomDimension) {
			var zoom = (contentSize.x <= contentSize.y) ? Seadragon.Config.minZoomDimension
					/ containerSize.x
					: Seadragon.Config.minZoomDimension
	...
	function getMaxZoom() {
		var zoom = contentSize.x * Seadragon.Config.maxZoomPixelRatio
				/ containerSize.x;
	...
	this.getBounds = function(current) {
		...
		return new Seadragon.Rect(center.x - width / 2.0, center.y - height
				/ 2.0, width, height);
	};
	this.getCenter = function(current) {
		...
		var newZoomPixel = zoomPoint.minus(bounds.getTopLeft()).times(
	...
	this.applyConstraints = function(immediately) {
		...
		var constrainedZoom = Math.max(Math.min(actualZoom, getMaxZoom()),
				getMinZoom());
	...
	this.fitBounds = function(bounds, immediately) {
		...
		var newBounds = new Seadragon.Rect(bounds.x, bounds.y, bounds.width,
				bounds.height);
		...
		var refPoint = oldBounds.getTopLeft().times(
				containerSize.x / oldBounds.width)
				.minus(
						newBounds.getTopLeft().times(
								containerSize.x / newBounds.width)).divide(
						containerSize.x / oldBounds.width - containerSize.x
								/ newBounds.width);
	...
	this.goHome = function(immediately) {
		...
		if (Seadragon.Config.wrapVertical) {
			center.y = (contentHeight + (center.y % contentHeight))
					% contentHeight;
	...
	this.resize = function(newContainerSize, maintain) {
		...
		containerSize = new Seadragon.Point(newContainerSize.x,
				newContainerSize.y);
		if (maintain) {
	...
	this.update = function() {
		...
		if (zoomPoint && zoomSpring.getCurrent() != oldZoom) {
			...
			var deltaZoomPixels = newZoomPixel.minus(oldZoomPixel);
			var deltaZoomPoints = self.deltaPointsFromPixels(deltaZoomPixels,
					true);
	...
	this.pointFromPixel = function(pixel, current) {
		var bounds = self.getBounds(current);
		return pixel.divide(containerSize.x / bounds.width).plus(
				bounds.getTopLeft());
	};
	init();
};
(function() {
	...
	var useCanvas = typeof (document.createElement("canvas").getContext) == "function"
			&& subpixelRenders;
	var MS_INTERPOLATION_MODE = (typeof document.documentMode !== "undefined") ? "bicubic"
			: "nearest-neighbor";
	...
	function Overlay(elmt, loc, placement) {
		...
		this.adjust = createAdjustmentFunction(loc instanceof Seadragon.Point ? placement
				: Placement.TOP_LEFT);
	...
	Seadragon.Drawer = function(source, viewport, elmt) {
		...
		var canvas = Seadragon.Utils.makeNeutralElement(useCanvas ? "canvas"
				: "div");
		...
		function getTile(level, x, y, time, numTilesX, numTilesY) {
			...
			if (!tilesMatrix[level][x][y]) {
				...
				tilesMatrix[level][x][y] = new Tile(level, x, y, bounds,
						exists, url);
			}
		...
		function loadTile(tile, time) {
			tile.loading = imageLoader.loadImage(tile.url, Seadragon.Utils
					.createCallback(null, onTileLoad, tile, time));
		}
		function onTileLoad(tile, time, image) {
			tile.loading = false;
			if (midUpdate) {
				Seadragon.Debug
						.error("Tile load callback in middle of drawing routine.");
				return;
			} else if (!image) {
				Seadragon.Debug.log("Tile " + tile + " failed to load: "
						+ tile.url);
				tile.exists = false;
				return;
			} else if (time < lastResetTime) {
				Seadragon.Debug.log("Ignoring tile " + tile
						+ " loaded before reset: " + tile.url);
				return;
			}
			tile.loaded = true;
			tile.image = image;
		...
		function isCovered(level, x, y) {
			if (x === undefined || y === undefined) {
				...
				return (providesCoverage(level + 1, 2 * x, 2 * y)
						...
						&& providesCoverage(level + 1, 2 * x + 1, 2 * y) && providesCoverage(
						level + 1, 2 * x + 1, 2 * y + 1));
			}
		}
		...
		function updateActual() {
			...
			var lowestLevel = _max(minLevel, _floor(_log(minDimension)
					/ _log(2)));
			var zeroRatioC = _deltaPixelsFromPoints(_getPixelRatio(0), true).x;
			var highestLevel = _min(maxLevel, _floor(_log(zeroRatioC
					/ MIN_PIXEL_RATIO)
					/ _log(2)));
			...
			for ( var level = highestLevel; level >= lowestLevel; level--) {
				...
				for ( var x = tileTL.x; x <= tileBR.x; x++) {
					for ( var y = tileTL.y; y <= tileBR.y; y++) {
						var tile = _getTile(level, x, y, currentTime,
								numTilesX, numTilesY);
						...
						var tileDistance = viewportCenterPixel
...
(function() {
	...
	function makeNavControl(viewer) {
		...
		var zoomIn = new Seadragon.Button(Seadragon.Strings
				.getString("Tooltips.ZoomIn"), url(ZOOM_IN, REST), url(ZOOM_IN,
				GROUP), url(ZOOM_IN, HOVER), url(ZOOM_IN, DOWN),
				beginZoomingIn, endZooming, doSingleZoomIn, beginZoomingIn,
				endZooming);
		var zoomOut = new Seadragon.Button(Seadragon.Strings
				.getString("Tooltips.ZoomOut"), url(ZOOM_OUT, REST), url(
				ZOOM_OUT, GROUP), url(ZOOM_OUT, HOVER), url(ZOOM_OUT, DOWN),
				beginZoomingOut, endZooming, doSingleZoomOut, beginZoomingOut,
				endZooming);
		var goHome = new Seadragon.Button(Seadragon.Strings
				.getString("Tooltips.Home"), url(HOME, REST), url(HOME, GROUP),
				url(HOME, HOVER), url(HOME, DOWN), null, onHome, null, null,
				null);
		var fullPage = new Seadragon.Button(Seadragon.Strings
				.getString("Tooltips.FullPage"), url(FULL_PAGE, REST), url(
				FULL_PAGE, GROUP), url(FULL_PAGE, HOVER), url(FULL_PAGE, DOWN),
				null, onFullPage, null, null, null);
		group = new Seadragon.ButtonGroup(
				[ zoomIn, zoomOut, goHome, fullPage ]);
	...
	Seadragon.Viewer = function(container) {
			...
			window.setTimeout(
					function() {
						if (lastOpenStartTime > lastOpenEndTime) {
							setMessage(Seadragon.Strings
									.getString("Messages.Loading"));
						}
					}, 2000);
		...
		function onOpen(time, _source, error) {
			...
			source = _source;
			viewport = new Seadragon.Viewport(prevContainerSize,
					source.dimensions);
		...
		function onContainerEnter(tracker, position, buttonDownElmt,
				buttonDownAny) {
			mouseInside = true;
			abortControlsAutoHide();
		}
		function onContainerExit(tracker, position, buttonDownElmt,
				buttonDownAny) {
			if (!buttonDownElmt) {
				mouseInside = false;
				if (!animating) {
		...
		function onContainerRelease(tracker, position, insideElmtPress,
				insideElmtRelease) {
			if (!insideElmtRelease) {
				mouseInside = false;
				if (!animating) {
		...
		function onCanvasClick(tracker, position, quick, shift) {
			if (viewport && quick) {
				...
				viewport
						.zoomBy(factor, viewport.pointFromPixel(position, true));
		...
		function onCanvasRelease(tracker, position, insideElmtPress,
				insideElmtRelease) {
		...
		this.openDzi = function(xmlUrl, xmlString) {
			...
			Seadragon.DziTileSource.createFromXml(xmlUrl, xmlString,
					Seadragon.Utils.createCallback(null, onOpen, currentTime));
		};
		...
		this.setFullPage = function(fullPage) {
			...
			if (viewport) {
				...
				if (fullPage) {
					fsBoundsDelta = new Seadragon.Point(newBounds.width
							/ oldBounds.width, newBounds.height
							/ oldBounds.height);
				} else {
					viewport.update();
